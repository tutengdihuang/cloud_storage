...
func (s *Store) upToken() string {
	...
	mac := qbox.NewMac(s.config.AccessKeyID, s.config.AccessKeySecret)
...
func (s *Store) getDownloadUrl(key string) string {
	...
	if s.config.Private {
		mac := qbox.NewMac(s.config.AccessKeySecret, s.config.AccessKeySecret)
